(function () {
  ...
  const tarjetaEspecialidad = (e) => `
    <article class="card">
      ...
      <button class="btn btn-azul" onclick="abrirModalConsulta('${e.id}')">Agendar cita</button>
    </article>`;
  ...
  const tarjetaCurso = (c) => `
    <article class="card">
      ...
      <button class="btn btn-oro" onclick="abrirModalCurso('${c.id}')">Inscribirme</button>
    </article>`;
  ...
  const sala = (nombres, inputs, frames) => {
    $$(".sala-tabs button").forEach((b) => b.classList.remove("activo"));
    $$(".sala-panel").forEach((p) => p.classList.remove("activo"));
    $("#tab-" + nombres).classList.add("activo");
    $("#panel-" + nombres).classList.add("activo");
  };

  window.cambiarSala = (nombre) => sala(nombre);

  const conectar = (inputId, frameId, nombre) => {
  ...
  window.unirseConsulta = () => conectar("link-consulta", "frame-consulta");
  window.unirseCurso = () => conectar("link-curso", "frame-curso");
